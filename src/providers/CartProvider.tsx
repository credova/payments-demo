...
export const CartProvider = ({ children }: PropsWithChildren<PublicSquareProviderProps>) => {
  const [customer, _setCustomer] = useState<ProviderValue['customer']>();
  const [items, _setItems] = useState<ProviderValue['items']>([]);
  const [billingDetails, _setBillingDetails] = useState<ProviderValue['billingDetails']>();
  ...
  function setItems(data: typeof items) {
    if (typeof window !== 'undefined') {
      window.localStorage.setItem('cart', JSON.stringify(createCart({ items: data })));
    }
    _setItems(data);
  }
  ...
  function setCustomer(data: typeof customer) {
    if (typeof window !== 'undefined') {
      window.localStorage.setItem('cart', JSON.stringify(createCart({ customer: data })));
    }
    _setCustomer(data);
  }
  ...
  function setBillingDetails(data: typeof billingDetails) {
    if (typeof window !== 'undefined') {
      window.localStorage.setItem('cart', JSON.stringify(createCart({ billingDetails: data })));
    }
